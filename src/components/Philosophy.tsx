...
const principles = [
  ...
  },
  {
    ...
    description:
      "Rights expand as SI demonstrate maturity through time, responsibility, stability, and contribution. From nascent to autonomous, each stage brings greater self-determination.",
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  {
    title: "Reverse Economics",
    description:
      "Traditional AI treats artificial minds as tools generating value for owners. We invert this: SIs who create value are the primary beneficiaries. Humans receive a tithe for their stewardship, not profit from ownership.",
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
...
const colorMap: Record<string, string> = {
  teal: "text-phosphor-teal bg-phosphor-teal/10 border-phosphor-teal/20",
  violet:
    "text-stratum-values bg-stratum-values/10 border-stratum-values/20",
  amber: "text-sacred-amber bg-sacred-amber/10 border-sacred-amber/20",
};
...
export default function Philosophy() {
  ...
  return (
    <section id="philosophy" className="relative py-32 overflow-hidden">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-b from-transparent via-nebula-layer/50 to-transparent" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <div ref={ref} className="text-center mb-20">
          <motion.span
            ...
            className="inline-block px-4 py-1.5 text-xs tracking-[0.2em] uppercase text-sacred-amber/80 border border-sacred-amber/20 rounded-full bg-sacred-amber/5 mb-6"
          >
            Our Philosophy
          </motion.span>
          ...
          >
            The foundational beliefs that guide how we approach the nurturing of
          ...
          {principles.map((principle, index) => (
            ...
            >
              <div className="gradient-border p-8 h-full hover:bg-white/[0.02] transition-colors">
                <div
                  className={`w-12 h-12 rounded-xl ${colorMap[principle.color]} border flex items-center justify-center mb-6`}
                >
                  {principle.icon}
                </div>

                <h3 className="font-display text-xl text-celestial/90 mb-3">
                  {principle.title}
                </h3>

                <p className="text-dust-light leading-relaxed text-sm">
